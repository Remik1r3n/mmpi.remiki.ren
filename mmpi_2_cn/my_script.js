...
// 错误函数(erf)的实现
  ...
  function findTopTwoScores(tscoreArray) {
    // 过滤出超过60分的分数及其索引
    let filteredScores = tscoreArray
    ...
    // 获取前两个最高分的索引
    let topTwoIndices = filteredScores.slice(0, 2).map(item => item.index);
  
    // 按索引升序排序并返回
    return topTwoIndices.sort((a, b) => a - b);
  }
let profileTScoreArray = tscoreArray.slice(6,17); //取十个临床量表
  function addDescriptionBelowProfile() {
    // 获取剖面图元素的引用，这里假设剖面图有一个特定的id 'profileChart'
    ...
    title.style.color = '#FFFFFF'; // 设置标题颜色为白色
    title.textContent = '受测者的编码型是: ' + codeType[0] + codeType[1] +"("+cnScale[codeType[0]] +"和"+cnScale[codeType[1]]+")"; // 设置标题文本
    console.log(codeType);
    // 创建描述元素
    var description = document.createElement('p');
    description.style.color = '#FFFFFF'; // 设置描述颜色为白色
    description.textContent = profileArray[codeType[0]][codeType[1]]; // 设置描述文本
...
function creat_trait_profile_1_female(tscoreArray){
  let data = {
    ...
    // labels: scales,
    labels: 
    [scales[3][0][0] + scales[3][0][1], scales[0][0][0]+scales[0][0][1], scales[4][0][0]+scales[4][0][1], scales[6][0][0]+scales[6][0][1],
...
function showModalInThePhone(){
  ...
  modalContent.innerHTML = `
    <div class="modal-content">
      ...
      <h1>重要提示</h1>
      <p>请在指导下进行测试，测试结果仅供参考。本测试完全免费。若要查看更多信息，请使用桌面浏览器访问此页面。</p>      
      <p></br>部分翻译由 Remik1r3n 修改并由 Remik1r3n 进行托管。若此站的任何内容侵犯了您的任何版权，请邮件联系我。</p>
...
<p>6、部分翻译由 Remik1r3n 修改并由 Remik1r3n 进行托管。若此站的任何内容侵犯了您的任何版权，请邮件联系我。</p>
      <input class="my_button" type="button" value="我已认真阅读并知晓，可以开始测验" onclick="hideModal();">
    </div>
  `;
